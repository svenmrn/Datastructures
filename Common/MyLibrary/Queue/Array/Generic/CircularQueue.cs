...
namespace MyLibrary.Queue.Array.Generic
{
    ...
    public class CircularQueue<T>
    {
        ...
        public CircularQueue(int InitialLength, bool AutoResize = true)
        {
        ...
        public T Dequeue()
        {
            ...
            queue[Front] = default(T);                   //not required, but for easier debugging
        ...
        public T[] Queue
        {
            get
            {
                return (T[])queue.Clone();    // We return a clone (=copy of the array in an new array) so that the queue cannot be changed by the caller
            }
        }
